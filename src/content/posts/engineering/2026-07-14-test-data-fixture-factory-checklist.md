...
author: "BLOA Team"
date: "2026-07-14"
summary: "테스트 코드가 늘어날수록 테스트 데이터 관리가 더 어려워진다. Fixture, Factory, Builder, seed 데이터를 언제 나눠 써야 하는지와 E2E 테스트에서 계정·상태를 다루는 기준을 정리했다."
...
결론부터 말하면, **테스트 데이터는 한 가지 방식으로 통일하는 것보다 목적별로 나누는 편이 낫습니다.** 변하지 않는 기준 데이터는 Fixture나 seed가 맞고, 케이스마다 조건이 달라지는 도메인 데이터는 Factory나 Test Data Builder가 낫습니다. 로그인 상태와 브라우저 세션처럼 민감한 E2E 데이터는 별도 파일로 저장하되, 저장소에 올리지 않는 규칙까지 함께 정해야 합니다.

> 요약 박스
>
> - Fixture는 작고 변하지 않는 기준 데이터에만 쓰는 편이 좋습니다.
> - Factory는 테스트마다 필요한 사용자의 상태, 주문 조건, 권한 조합을 빠르게 만들 때 유리합니다.
> - Test Data Builder는 "기본값은 감추고, 이 테스트에서 중요한 차이만 드러내는" 구조에 맞습니다.
> - E2E 인증 상태 파일은 쿠키와 헤더를 담을 수 있으므로 `.gitignore`에 넣고 만료·재생성 절차를 정해야 합니다.

최종 업데이트: 2026-07-14

이 글은 Jest, Playwright, Laravel Factory 문서와 Martin Fowler의 Test Data Builder 설명을 바탕으로 정리했습니다. 특정 언어나 프레임워크 하나에 묶기보다, 테스트 데이터가 커질 때 유지보수 비용을 줄이는 판단 기준에 초점을 맞췄습니다.
...
반대로 사용자, 주문, 결제 같은 도메인 객체는 Fixture 하나로 오래 버티기 어렵습니다. 테스트마다 "일반 사용자", "정지된 사용자", "이메일 미인증 사용자", "구독 만료 사용자"가 필요해지고, 결국 같은 파일 안에서 서로 다른 케이스가 뒤섞입니다.
...
로그인 시간을 줄이려고 인증 상태 파일을 쓰는 건 좋은 선택일 수 있습니다. 다만 그 파일은 테스트 데이터이면서 동시에 민감 정보입니다. 일반 Fixture처럼 저장소에 올리는 순간 보안 사고가 될 수 있습니다.
...
여기서 중요한 건 도구 이름이 아닙니다. 같은 팀 안에서 "무엇은 공유하고, 무엇은 테스트마다 만든다"는 기준이 같아야 합니다.
...
## 정리: 테스트 데이터는 적게 쓰는 게 아니라 가까이 둬야 합니다

테스트 데이터 관리는 "Fixture를 쓸까, Factory를 쓸까"라는 도구 선택으로 끝나지 않습니다. 더 중요한 질문은 이겁니다. 이 테스트가 왜 이 데이터로 실행되는지, 다음 사람이 파일을 열었을 때 바로 알 수 있는가.

고정 기준 데이터는 Fixture나 seed로 작게 유지하세요. 케이스마다 달라지는 데이터는 Factory나 Builder로 테스트 가까이에 두는 편이 낫습니다. E2E 인증 상태처럼 민감한 데이터는 속도보다 보안과 재생성 절차를 먼저 정해야 합니다. 테스트가 실패했을 때 데이터 조건을 1분 안에 설명할 수 있다면, 그 테스트는 오래 버틸 가능성이 높습니다.
