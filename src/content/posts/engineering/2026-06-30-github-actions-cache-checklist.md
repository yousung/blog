...
author: "감성개발자"
date: "2026-06-30"
summary: "GitHub Actions 워크플로가 느릴 때 무작정 캐시를 추가하면 오히려 원인 파악이 어려워질 수 있다. 의존성 설치, 빌드 산출물, 캐시 키, restore-keys, 비용까지 실무 기준으로 훑어봤다."
...
"CI 한 번 도는 데 8분." 브랜치 하나 올릴 때마다 초록불 기다리며 딴짓을 하다 보면 누구나 같은 생각을 합니다. 이거 캐시로 어떻게 안 되나. `node_modules`를 저장하면 빨라질 것 같고, 빌드 폴더를 통째로 올리면 다음 실행이 금방 끝날 것 같습니다.

그런데 캐시는 잘못 넣으면 속도보다 혼란을 먼저 만듭니다. 오래된 의존성이 살아나거나, 캐시 복구 시간 때문에 오히려 느려지거나, 키가 자주 바뀌어 매번 새 캐시만 쌓이는 식입니다. 넘겨받은 워크플로에서 "왜인지 모르게 가끔 깨지는 CI"를 파고들면, 십중팔구 범위가 너무 넓게 잡힌 캐시가 원인이었습니다.

그래서 내 기준은 하나입니다. **GitHub Actions 캐시는 "매번 같은 비용을 반복해서 내는 구간"에만 넣습니다.** 의존성 다운로드처럼 입력이 명확하고 재사용 가능성이 높은 작업에는 잘 맞습니다. 반대로 테스트 결과, 임시 빌드 산출물, 환경마다 달라지는 파일을 무리하게 저장하면 디버깅 비용이 더 커질 수 있습니다.

바쁜 분들을 위해 먼저 요약하면 이렇습니다.
...
만약 lock 파일이 아예 없어서 `hashFiles()`에 넣을 게 없다면, 키를 고민하기 전에 lock 파일을 쓰는 쪽부터 검토하는 게 낫습니다. CI에서 의존성 재현성을 잡으려면 lock 파일이 기준점이 되고, `npm ci`도 `package-lock.json`이나 `npm-shrinkwrap.json`이 있어야 동작하기 때문입니다. (출처: [npm Docs - npm ci](https://docs.npmjs.com/cli/v9/commands/npm-ci/))
...
`node_modules` 전체 캐시가 맞는 경우도 있습니다. 설치 시간이 지나치게 길고, OS·런타임·lock 파일이 안정적이며, 실패 시 캐시를 쉽게 지울 수 있는 운영 절차가 있다면 검토할 수 있습니다. 하지만 작은 팀이나 개인 프로젝트라면 먼저 단순한 캐시부터 적용하고, 실행 시간 비교표를 남기는 쪽이 낫습니다. 나는 팀 규모가 작을수록 `node_modules` 통째 캐시는 기본에서 뺍니다. 빨라져 봐야 몇십 초인데, 한 번 어긋나면 원인을 찾느라 반나절이 날아가는 걸 몇 번 겪었기 때문입니다.
...
개인 블로그, 작은 API, 테스트가 몇 초 안에 끝나는 프로젝트라면 캐시가 체감되지 않을 수 있습니다. 오히려 캐시 복구와 저장 로그가 늘어나면서 workflow만 복잡해집니다. 이 경우에는 `npm ci`, lock 파일 관리, 불필요한 job 제거, `paths` 필터 같은 기본 정리가 먼저입니다. 나는 이런 규모에서는 캐시를 아예 나중 과제로 미룹니다. 유지보수할 사람이 적을수록, 빠른 CI보다 읽어서 바로 이해되는 CI가 더 값집니다.
