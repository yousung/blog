...
![저장소 밖으로 이미 흩어져 회수할 수 없는 옛 키 복제본들이, 자물쇠의 내부 형태가 새로 바뀌자 더 이상 맞지 않아 어둡게 무력화되는 과정을 형태 대비로 보여주는 도식. 유출된 값은 지워서 되돌릴 수 없지만 키 교체가 그 복제본들을 못 쓰게 만든다는 순서를 시각화한다.](/images/posts/env-secret-management-guide/figure.png)
