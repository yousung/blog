...
![넓은 화면에서는 본문 열 옆에 나란히 정렬돼 있던 사이드바 슬롯이, 좁은 화면으로 바뀌면 본문이 시작될 자리로 파고들어 본문 블록들을 아래로 밀어내고 맨 아래 블록이 화면 밖으로 잘려 사라지는 반응형 재배치 실패를, 두 기기 프레임 대비로 보여주는 도식.](/images/posts/design-application-audit-checklist/figure.png)
