...
![하나뿐인 상위 레코드가 여러 하위 레코드와 결합되면서 결합 결과의 모든 행마다 똑같이 복제되어, 아래로 쌓인 결과 더미가 불필요하게 무겁게 부풀어 오르는 모습을 형태와 흐름만으로 나타낸 도식](/images/posts/orm-n-plus-one-query/figure.png)
