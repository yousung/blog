...
author: "BLOA Team"
date: "2026-07-21"
summary: "브라우저와 Node.js에서 fetch 호출이 오래 멈추거나 실패할 때 타임아웃, AbortController, HTTP 상태 코드, 재시도 기준을 어떻게 나눠 봐야 하는지 실무 체크리스트로 정리했다."
...
결론부터 말하면, **`fetch`는 타임아웃, 취소, HTTP 오류, 재시도를 서로 다른 문제로 나눠 처리해야 합니다.** 네트워크 실패는 `catch`로 들어오지만, `404`나 `500` 같은 HTTP 상태 코드는 응답 객체로 정상 도착할 수 있습니다. 재시도도 모든 요청에 붙이면 안 됩니다. 같은 `POST`를 두 번 보내면 주문, 결제, 가입 요청이 중복될 수 있기 때문입니다.
...
이 글은 브라우저 Fetch API, Node.js의 `AbortSignal`, web.dev의 fetch 오류 처리 가이드, HTTP Semantics RFC 9110을 바탕으로 정리했습니다. 특정 라이브러리보다 실무에서 API 호출 실패를 분류하는 기준에 초점을 맞췄습니다.
...
다만 타임아웃 숫자는 정답이 없습니다. 목록 조회와 파일 업로드를 같은 5초로 묶으면 한쪽은 너무 느슨하고, 다른 한쪽은 너무 빡빡합니다. 기준은 기능별로 잡아야 합니다.
...
재시도 횟수도 작게 시작하는 편이 좋습니다. 보통은 1~2회만으로도 일시적 네트워크 흔들림을 흡수할 수 있습니다. 그 이상은 사용자의 대기 시간을 늘리고 서버 상태를 더 나쁘게 만들 수 있습니다.
...
## 정리: 실패를 한 덩어리로 보지 않는 것이 먼저입니다

API 호출 안정성은 재시도 횟수를 늘리는 문제로 끝나지 않습니다. 타임아웃은 사용자가 기다릴 수 있는 시간을 정하는 일이고, 취소는 더 이상 필요 없는 요청을 정리하는 일입니다. HTTP 오류는 응답을 받았지만 원하는 결과가 아닌 상태이고, 재시도는 같은 요청을 다시 보내도 되는지 확인한 뒤에야 붙일 수 있습니다.
